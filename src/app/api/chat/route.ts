...
const SYSTEM_PROMPT =
  ...
  "Be honest about what you don't know. You are unlimited and free to use.";
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        // ── Optional real-time web search (DuckDuckGo) ────────────────────
        let apiMessages = buildApiMessages(history);

        if (body.webSearch && userQuery.trim()) {
          send({ searching: true });
          ...
          if (search.abstract || search.results.length > 0) {
            ...
            // Insert fresh data as a system note right before the latest turn.
            apiMessages = [
              ...apiMessages.slice(0, -1), // existing system + history
              { role: "system", content: searchContext },
              apiMessages[apiMessages.length - 1], // the latest user turn
            ];
          }
        }
