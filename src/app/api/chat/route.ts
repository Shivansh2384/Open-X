...
import { getRealtimeEnrichment } from "@/lib/search/realtime";
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      try {
        // ── Real-time data enrichment + web search ────────────────────────
        ...
        if (shouldSearch) {
          ...
          // Run realtime enrichment (time API, etc.) and web search in parallel
          const [enrichment, search] = await Promise.all([
            getRealtimeEnrichment(userQuery),
            searchDuckDuckGo(userQuery),
          ]);

          // Build the context to inject into the user's message
          const contextParts: string[] = [];

          // Realtime enrichment always takes priority (accurate API data)
          if (enrichment) {
            contextParts.push(enrichment.data);
          ...
          if (search.abstract || search.results.length > 0) {
            ...
            contextParts.push(formatSearchForPrompt(search));
            enrichedWithData = true;
          }

          // CRITICAL: merge data INTO the user's message (not a 2nd system
          // message). Reasoning models only honor the first system message,
          // so a separate one gets ignored.
          if (enrichedWithData) {
